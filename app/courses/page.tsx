...
export default function CourseCatalog() {
  ...
  return (
    <>
      <Navbar />
      <main className="flex flex-1 flex-col md:flex-row max-w-400 mx-auto w-full px-4 md:px-6 lg:px-8 py-6 gap-8 min-h-screen">
        
        {/* --- SIDEBAR FILTERS --- */}
        <aside className="w-full md:w-1/4 lg:w-70 shrink-0 flex flex-col gap-6">
          <div className="flex items-center justify-between md:hidden pb-4 border-b border-slate-200 dark:border-surface-highlight">
            ...
            <div className="flex flex-col gap-3">
               ...
               <div className="relative">
                 ...
                 <input 
                    ...
                    className="w-full pl-9 pr-3 py-2 rounded-lg bg-white dark:bg-background-dark border border-slate-200 dark:border-[#3d4052] text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary"
                    value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                 />
               </div>
            </div>
            <div className="h-px bg-slate-200 dark:bg-surface-highlight w-full"></div>
              ...
              <div className="flex flex-col gap-1">
                {["Design", "Development"].map((cat) => (
                  <label key={cat} className="flex items-center gap-3 py-2 cursor-pointer group">
                    <input checked={selectedCategories.includes(cat)} onChange={() => toggleCategory(cat)} className="h-5 w-5 rounded border-slate-300 dark:border-[#3d4052] bg-white dark:bg-transparent text-primary focus:ring-primary/20 dark:focus:ring-offset-background-dark" type="checkbox"/>
              ...
              <div className="flex flex-col gap-2">
                {["All", "Beginner", "Intermediate", "Expert"].map((level) => (
                  <label key={level} className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-all ${selectedLevel === level ? 'border-primary bg-primary/5 dark:bg-primary/10' : 'border-slate-200 dark:border-[#3d4052] hover:bg-slate-50 dark:hover:bg-[#1e212b]'}`}>
                    <input checked={selectedLevel === level} onChange={() => handleLevelChange(level)} className="h-4 w-4 border-slate-300 dark:border-[#3d4052] bg-transparent text-primary focus:ring-primary/20 dark:focus:ring-offset-background-dark" name="level" type="radio"/>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="appearance-none bg-white dark:bg-surface-highlight border border-slate-200 dark:border-[#3d4052] text-slate-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5 pr-8 cursor-pointer outline-none shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
              {filteredAndSortedCourses.map((course) => (
                <Link href={`/courses/${course.id}`} key={course.id} className="cursor-pointer">
                  <div className="group flex flex-col bg-white dark:bg-[#1e212b] rounded-xl overflow-hidden border border-slate-200 dark:border-surface-highlight hover:shadow-xl hover:border-primary/30 dark:hover:border-primary/50 transition-all duration-300 hover:-translate-y-1 h-full">
                    <div className="relative aspect-video w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                      ...
                      <Image alt={course.title} fill className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" src={course.image} />
                    ...
                    <div className="flex flex-col flex-1 p-5 gap-3">
                      ...
                      <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-100 dark:border-surface-highlight">
                        ...
                        <button className="flex items-center justify-center h-9 px-4 rounded-lg bg-primary hover:bg-primary/90 text-white text-sm font-medium transition-colors shadow-sm cursor-pointer">View Course</button>
                      </div>
                    </div>
                  </div>
                </Link>
